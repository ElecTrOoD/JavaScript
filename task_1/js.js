...
function myGame() {
    ...
    const game = {
        ...
        getBarrier() {
            this.barrier.reset()
            for(let i = 1; i < this.config.getBarrierCount(); i++) {
                this.barrier.pushBarriers(this.getRandomFreeCoordinates())
            }
        },

        getRandomFreeCoordinates() {
            const exclude = [this.food.getCoordinates(), ...this.snake.getBody(), ...this.barrier.getPoints()];
        ...
        tickHandler() {
            ...
            if (this.food.isOnPoint(this.snake.getNextStepHeadPoint())) {
                ...
                if (this.isGameWon()) {
                    this.finish();
                    this.score.winFoodCountReached();
                }
            }
        ...
        render() {
            this.map.render(this.snake.getBody(), this.food.getCoordinates(), this.barrier.getPoints());
        }
    };

    return {
        init(userSettings) {
            game.init(userSettings);
        }
    };
}

const snakeGame = myGame()
snakeGame.init({speed: 8, winFoodCount: 5})
